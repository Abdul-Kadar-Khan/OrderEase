...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    ...
    for (const item of targetLineItems) {
      ...
      if (state.blocked) {
        // A checkout discount that specifically targets this product (or
        // something we can't safely reason about) — leave it alone.
        // Note: an order-wide checkout discount does NOT hit this branch;
        // it's fine to add a product-level discount alongside it (see below).
        // warnings.push(
        //   `"${lineItemDisplayName(item)}" already has a discount applied during checkout that targets this specific product. Checkout discounts like this cannot be modified here.`,
        // );
        warnings.push(
          `The product already has a discount applied.`,
        );
        continue;
      }
      ...
      // Only replace an existing APP-applied product discount when the new
      // one is strictly greater. Otherwise leave it exactly as it is. This
      // never applies to a checkout-origin order-wide discount, since that
      // isn't a "product-level" discount to begin with (tag.productAmount
      // stays 0 for it).
      if (state.tag.productAmount > 0 && newProductAmount <= state.tag.productAmount) {
        ...
        warnings.push(
          `"${lineItemDisplayName(item)}" already has ${existingLabel} applied, worth ${state.tag.productAmount.toFixed(2)} ${currencyCode}. ` +
            `"${discountCode}" would only be worth ${newProductAmount.toFixed(2)} ${currencyCode} on this product, so it was not applied — the existing discount is greater than or equal and was kept as-is.`,
        );
        continue;
      }
      ...
      // Only remove an existing discount application when it's one THIS
      // app created. A checkout-origin discount (including an order-wide
      // one we're allowed to keep) can't be removed via the Order Editing
      // API, and — for the order-wide case — doesn't need to be: we just
      // add our own separate product-level discount alongside it.
      if (state.existingApplicationId && state.existingIsOurs) {
        ...
        if (removeJson.errors?.length || removeErrors.length) {
          ...
          // Belt-and-suspenders: if Shopify itself is telling us this
          // discount can't be removed because it came from a discount
          // code, treat it the same way we treat a known protected
          // discount — a friendly warning, not a raw API error — even if
          // our own origin classification above didn't catch it.
          if (/discount code/i.test(rawMessage) && /can'?t be removed/i.test(rawMessage)) {
            ...
            // );
            warnings.push(
              "This product already has a discount applied.",
            );
          } else {
      ...
      // Our own fixedValue only ever covers the product-level portion (plus
      // any legacy order-level component this line item may already be
      // carrying from before this route stopped supporting order-level
      // discounts). It deliberately does NOT include state.tag.checkoutAmount
      // — that belongs to a separate, still-existing checkout discount
      // application that we aren't touching.
      ...
      if (applyJson.errors?.length || applyErrors.length) {
        ...
        if (state.tag.checkoutAmount > 0 && /discount/i.test(rawMessage)) {
          // We tried to add alongside an existing order-wide checkout
          // discount and Shopify's order-editing API rejected it — some
          // stores/API versions don't allow stacking at all. Surface this
          // clearly rather than as a generic error.
          warnings.push(
            "This product already has an order-level discount applied from checkout, and Shopify would not allow an additional discount to be added on top of it here.",
          );
        } else {
