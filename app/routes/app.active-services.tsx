...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (intent === "saveMaxEdits") {
    ...
    console.log(
      `[ActiveServices Action] Saving max edits setting: shop=${shop}, maxEdits=${maxEdits}`,
    );
...
export default function ActiveServicesPage(): JSX.Element {
  const { services, timeLimitSettings, maxEditsSettings } = useLoaderData<typeof loader>();
  ...
  return (
    <s-page heading="Active Services">
      ...
      <MaxEditsSection initialMaxEdits={maxEditsSettings.maxEdits} />
...
function MaxEditsSection({ initialMaxEdits }: MaxEditsSectionProps): JSX.Element {
  const fetcher = useFetcher();
  const [selectedPreset, setSelectedPreset] = useEffectState(
    MAX_EDIT_PRESETS.some((p) => p.value === String(initialMaxEdits))
      ? String(initialMaxEdits)
      : "custom",
  );
  const [customEditsVal, setCustomEditsVal] = useEffectState(
    initialMaxEdits > 0 ? initialMaxEdits : 3,
  );
  const [isSaved, setIsSaved] = useEffectState(false);
  ...
  const handleSave = (preset: string, val?: number) => {
    let finalVal = 3;
    if (preset === "custom") {
  ...
  return (
    <s-section heading="Maximum Order Edits Limit">
      <s-box
        padding="large"
        border="base"
        borderRadius="base"
      ...
      >
        <s-stack direction="block" gap="large">
          <s-stack direction="block" gap="small">
            ...
            <s-paragraph color="subdued">
              Set the maximum number of order modifications allowed per order across all features (adding items, changing quantities, applying discounts, shipping address updates, etc.). PDF Invoice downloading is always permitted without restriction.
            </s-paragraph>
          </s-stack>
          ...
          {/* Custom Edit Limit Input */}
          {selectedPreset === "custom" && (
            <s-box padding="base" border="base" borderRadius="base">
              <s-stack direction="block" gap="base">
                <s-text type="strong">Custom Max Edits</s-text>
                <s-grid gridTemplateColumns="1fr auto" gap="base" alignItems="center">
                  <s-stack direction="block" gap="small">
                    <s-text color="subdued">Maximum Number of Edits</s-text>
                    <input
                      type="number"
                      min="1"
          ...
          {isSaved && (
            <s-badge tone="success">
              Max edits limit saved successfully!
            </s-badge>
          )}
        </s-stack>
      </s-box>
